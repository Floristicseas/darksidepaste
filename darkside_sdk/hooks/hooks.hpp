...
namespace hooks
{
	...
	namespace show_crosshair
	{
		inline c_hook show;

		bool hk_draw_crosshair(void* a1);
	}

	namespace remove_flash
	{
		inline c_hook remove;

		void* hk_remove_flash(__int64 a1, int a2, __int64* a3, __int64 a4, float a5[4]);
	}

	namespace remove_smoke
	{
		inline c_hook remove;

		void* hk_remove_smoke(void* a1, void* a2, int a3, int a4, void* a5, void* a6, void* a7, void* a8, void* a9, void* a10);
	}

	namespace team_intro_remove
	{
		inline c_hook remove;

		void hk_intro_remove(void* a1, void* a2, bool* pbTeamIntroEnd);
	}

	namespace overhead_info
	{
		inline c_hook remove;

		bool hk_overhead_remove(c_cs_player_pawn* a1);
	}
	...
	namespace glow_manager
	{
		...
		void hk_is_glowing(CGlowProperty* pGlow);
	}
	...
	namespace viewmodel
	{
		...
		void* hk_calc_viewmodel(float* flUnk, float* flOffsets, float* flFov);
	}

	namespace events
	{
		inline c_hook manager;

		void* event(float* flUnk, float* flOffsets, float* flFov);
	}
