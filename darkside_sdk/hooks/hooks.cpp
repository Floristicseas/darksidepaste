...
bool c_hooks::initialize( ) {
	...
	show_crosshair::show.hook(g_opcodes->scan(g_modules->m_modules.client_dll.get_name(), "48 89 5C 24 ? 57 48 83 EC 20 48 8B D9 E8 ? ? ? ? 48 85 C0 0F 84 ? ? ? ? BA"), show_crosshair::hk_draw_crosshair);

	remove_flash::remove.hook(g_opcodes->scan(g_modules->m_modules.client_dll.get_name(), "85 D2 0F 88 ? ? ? ? 48 89 4C 24 ?"), remove_flash::hk_remove_flash);

	remove_smoke::remove.hook(g_opcodes->scan(g_modules->m_modules.client_dll.get_name(), "48 89 5C 24 ? 48 89 6C 24 ? 48 89 74 24 ? 57 41 56 41 57 48 83 EC ? 48 8B 9C 24 ? ? ? ? 4D 8B F8"), remove_smoke::hk_remove_smoke);

	team_intro_remove::remove.hook(g_opcodes->scan(g_modules->m_modules.client_dll.get_name(), "48 83 EC 28 45 0F B6 08"), team_intro_remove::hk_intro_remove);

	overhead_info::remove.hook(g_opcodes->scan(g_modules->m_modules.client_dll.get_name(), "40 53 48 83 EC ? 48 8B D9 83 FA ? 75 ? 48 8B 0D ? ? ? ? 48 8D 54 24 ? 48 8B 01 FF 90 ? ? ? ? 8B 10"), overhead_info::hk_overhead_remove);

	viewmodel::calc.hook(g_opcodes->scan(g_modules->m_modules.client_dll.get_name(), "40 55 53 56 41 56 41 57 48 8B EC 48 83"), viewmodel::hk_calc_viewmodel);
...
void c_hooks::destroy( ) {
	...
	show_crosshair::show.unhook();
	remove_flash::remove.unhook();
	remove_smoke::remove.unhook();
	team_intro_remove::remove.unhook();
	overhead_info::remove.unhook();
	viewmodel::calc.unhook();
	glow_manager::glow.unhook();
...
void* hooks::override_view::hk_override_view(void* source_to_client, c_view_setup* view_setup)
{
	...
	if (g_cfg->misc.m_aspect_ratio > 0.0f)
	{
	...
	{
		view_setup->m_angles = g_interfaces->m_csgo_input->get_view_angles();
	}
...
void hooks::draw_scope_overlay::hk_draw_scope_overlay( void* a1, void* a2 ) {
	static auto original = m_draw_scope_overlay.get_original< decltype(&hk_draw_scope_overlay) >();
...
float hooks::get_field_of_view::hk_get_field_of_view( void* a1 ) {
	...
	return g_cfg->misc.m_field_of_view;
}

bool hooks::show_crosshair::hk_draw_crosshair(void* a1) {
	static auto original = show.get_original< decltype(&hk_draw_crosshair) >();

	if (g_cfg->misc.m_removals[2])
		return false;

	return original(a1);
}

void* hooks::remove_flash::hk_remove_flash(__int64 a1, int a2, __int64* a3, __int64 a4, float a5[4]) {
	static auto original = remove.get_original< decltype(&hk_remove_flash) >();

	if (g_cfg->misc.m_removals[3])
		return nullptr;

	return original(a1, a2, a3, a4, a5);
}

void* hooks::remove_smoke::hk_remove_smoke(void* a1, void* a2, int a3, int a4, void* a5, void* a6, void* a7, void* a8, void* a9, void* a10) {
	static auto original = remove.get_original< decltype(&hk_remove_smoke) >();

	if (g_cfg->misc.m_removals[4])
		return nullptr;

	return original(a1, a2, a3, a4, a5, a6, a7, a8, a9, a10);
}

void hooks::team_intro_remove::hk_intro_remove(void* a1, void* a2, bool* pbTeamIntroEnd) {
	static auto original = remove.get_original< decltype(&hk_intro_remove) >();

	if (g_cfg->misc.m_removals[5])
		return;

	return original(a1, a2, pbTeamIntroEnd);
}

bool hooks::overhead_info::hk_overhead_remove(c_cs_player_pawn* a1) {
	static auto original = remove.get_original< decltype(&hk_overhead_remove) >();
	
	if (!a1)
		return original(a1);

	if (a1 == g_ctx->m_local_pawn) {
		if (g_cfg->misc.m_removals[6])
			return false;
		return true;
	}
...
void hooks::glow_manager::hk_is_glowing(CGlowProperty* pGlow) {
	static auto original = glow.get_original< decltype(&hk_is_glowing) >();

	if (g_interfaces->m_engine->is_connected() || g_interfaces->m_engine->is_in_game() || !g_ctx->m_local_pawn)

	if (pGlow->m_pOwner->m_health() <= 0)
		return original(pGlow);

	if (!pGlow->m_pOwner->is_alive())
		return original(pGlow);

	if (g_ctx->m_local_pawn && pGlow->m_pOwner == g_ctx->m_local_pawn && g_key_handler->is_pressed(g_cfg->world_esp.m_thirdperson_key_bind, g_cfg->world_esp.m_thirdperson_key_bind_style)) {
		pGlow->m_glowColorOverride() = (g_cfg->visuals.glow.glow_color_local / 255);

		if (g_cfg->visuals.glow.m_bGlow_local)
			pGlow->m_bGlowing() = true;

	}

		if (g_ctx->m_local_pawn->m_team_num())
			return;

		if (g_ctx->m_local_pawn && (g_ctx->m_local_pawn == pGlow->m_pOwner || g_ctx->m_local_pawn->m_team_num() == pGlow->m_pOwner->m_team_num()))
			return original(pGlow);
	}

void* hooks::viewmodel::hk_calc_viewmodel(float* flUnk, float* flOffsets, float* flFov) {
	const auto original = calc.get_original< decltype(&hk_calc_viewmodel) >();

	void* pReturn = original(flUnk, flOffsets, flFov);

	if (!g_cfg->world_esp.viewchanger)
		return original(flUnk, flOffsets, flFov);

	float flXOffset = g_cfg->world_esp.flX;
	float flYOffset = g_cfg->world_esp.flY;
	float flZOffset = g_cfg->world_esp.flZ;
	float flXRotation = g_cfg->world_esp.flXrotation;
	float flYRotation = g_cfg->world_esp.flYrotation;
	float flZRotation = g_cfg->world_esp.flZrotation;
	float flViewmodelFov = g_cfg->world_esp.flfov;
...
void* hooks::object::hk_draw_object(void* pAnimatableSceneObjectDesc, void* pDx11, material_data_t* arrMeshDraw, int nDataCount, void* pSceneView, void* pSceneLayer, void* pUnk, void* pUnk2) {
	...
	if (!visuals::OnDrawObject(pAnimatableSceneObjectDesc, pDx11, arrMeshDraw, nDataCount, pSceneView, pSceneLayer, pUnk, pUnk2))
