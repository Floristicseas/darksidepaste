...
namespace crc32 {
	...
	static constexpr uint32_t hash_const( const char* _data, uint32_t len, uint32_t crc = 0 )
	{
	...
	static constexpr uint32_t hash_number_32( uint32_t number, uint32_t crc = 0 )
	{
		crc = crc ^ 0xFFFFFFFFU;

		for ( uint32_t i = 0; i < sizeof( uint32_t ); i++ )
		{
			crc = table[ static_cast<uint8_t>( number ) ^ ( crc & 0xFF ) ] ^ ( crc >> 8 );
			number >>= 8;
		}
