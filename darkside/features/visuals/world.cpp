...
void c_world::draw_scope_overlay()
{
    ...
    if (g_cfg->world_esp.custom_crosshair) {
        ...
        float fade_speed = g_cfg->world_esp.scope_lines_fade_speed;

        if (fade_speed > 3.f)
            m_alpha = std::clamp(m_alpha + frametime * fade_speed, 0.f, 1.f);
        else
            m_alpha = 1.f;

        float offset = g_cfg->world_esp.scope_lines_offset * (screen.y / 1080.f);
        float initial_pos = g_cfg->world_esp.scope_lines_initial_pos * (screen.y / 1080.f);

        c_color col = g_cfg->world_esp.scope_lines_color;
        col.a = static_cast<int>(col.a * m_alpha);

        g_render->gradient(
            vec3_t(screen.x / 2 - initial_pos + 2, screen.y / 2),
            vec3_t(initial_pos - offset, 1),
            c_color(col.r, col.g, col.b, 0),
            col, true
        );

        g_render->gradient(
            vec3_t(screen.x / 2 + offset, screen.y / 2),
            vec3_t(initial_pos - offset, 1),
            col,
            c_color(col.r, col.g, col.b, 0),
            true
        ...
        g_render->gradient(
            vec3_t(screen.x / 2, screen.y / 2 - initial_pos + 2),
            vec3_t(1, initial_pos - offset),
            c_color(col.r, col.g, col.b, 0),
            col, false
        );

        g_render->gradient(
            vec3_t(screen.x / 2, screen.y / 2 + offset),
            vec3_t(1, initial_pos - offset),
            col,
            c_color(col.r, col.g, col.b, 0),
            false
